...
-- Prediction calculation based on velocity
local function getTargetPrediction(targetPlayer, timeAhead)
    local character = targetPlayer.Character
    if character then
        local humanoid = character:FindFirstChild("Humanoid")
        if humanoid then
            local velocity = humanoid.RootPart.Velocity  -- Assuming RootPart for velocity calculation
            -- Predict future position by adding velocity scaled by time ahead
            local predictedPosition = character.HumanoidRootPart.Position + velocity * timeAhead
            ...
            if isMouseOnTarget(TargetPlayer) then
                ...
                for _, part in pairs(bodyParts) do
                    ...
                    if onScreen and isWithinViewEra(part.Position) then
                        ...
                        local predictedPosition = getTargetPrediction(TargetPlayer, 0.2)  -- Predict 0.2 seconds ahead
